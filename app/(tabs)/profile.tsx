...
function Profile({ previewSection }: { previewSection?: string } = {}) {
 ...
 const page = (selected: string, live: boolean) => {
  ...
  const body = <>
   ...
   <View style={styles.identity}>
     ...
     <View style={styles.identityRow}>
       ...
       <View style={styles.identityWords}>
         ...
         <View style={styles.nameRow}><LevelPill profile={profile}/>{profile.constraints.filter(c => c.active && c.kind === 'injury').map(c => <Text key={c.id} style={styles.injury}>⚕ {c.label}</Text>)}</View>
       </View>
     </View>
   ...
   <Pressable accessibilityRole="link" accessibilityLabel="Invite your hitting partners" onPress={() => router.push('/invite')} style={styles.health}><Ionicons name="person-add-outline" size={20} color={colors.brand}/><Text style={[styles.meta,{flex:1}]}>Invite your hitting partners</Text><Ionicons name="chevron-forward" size={16} color={colors.textMuted}/></Pressable>
