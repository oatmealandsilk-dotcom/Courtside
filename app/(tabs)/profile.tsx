...
export default function Profile({ previewSection }: { previewSection?: string } = {}) {
 ...
 const content = (selected: string) => {
   ...
   return <View style={{ minHeight: 320, backgroundColor: colors.bg }}>
     <View style={styles.grid}>{items.map(p => <Pressable key={p.id} accessibilityRole="link" accessibilityLabel={`Open ${p.kind}: ${p.body}`} onPress={() => router.push(`/post/${p.id}`)} style={[styles.tile, !p.thumbnailUrl && {backgroundColor: colors.brand}]}>{p.thumbnailUrl ? <><Image accessibilityIgnoresInvertColors source={{uri:p.thumbnailUrl}} style={StyleSheet.absoluteFill} resizeMode="cover"/><View pointerEvents="none" style={styles.tileScrim}/></> : null}<Ionicons name={p.kind==='reel'?'play':'document-text-outline'} size={18} color={p.thumbnailUrl ? '#FFFFFF' : colors.brandInk} style={{alignSelf:'flex-end'}}/><Text numberOfLines={4} style={[styles.tileText, {color: p.thumbnailUrl ? '#FFFFFF' : colors.brandInk}]}>{p.body}</Text><View style={styles.tileFoot}><Text style={[styles.tileLabel, {color: p.thumbnailUrl ? '#FFFFFF' : colors.brandInk}]}>{p.kind==='match'?'SET PLAY':p.kind.toUpperCase()}</Text><View style={styles.tileViews}><Ionicons name="eye-outline" size={11} color={p.thumbnailUrl ? '#FFFFFF' : colors.brandInk}/><Text style={[styles.tileLabel, {color: p.thumbnailUrl ? '#FFFFFF' : colors.brandInk}]}>{compactNumber(p.views ?? 0)}</Text></View></View></Pressable>)}</View>
 ...
 return <SwipeSurface enabled={!previewSection} onSwipe={direction => { if (direction === -1) router.navigate('/coaches'); }} renderPreview={direction => direction === -1 ? <Coaches/> : null}><Screen memoryKey="profile" title="Profile" subtitle={`@${user.handle}`} right={<View style={styles.headerActions}>
