...
export default function Profile({ previewSection }: { previewSection?: string } = {}) {
 ...
 return <SwipeSurface enabled={!previewSection} onSwipe={direction => { if (direction === -1) router.navigate('/coaches'); }} renderPreview={direction => direction === -1 ? <Coaches/> : null}><Screen memoryKey="profile" title="Profile" subtitle={`@${user.handle}`} right={<View style={styles.headerActions}>
   ...
   <Pressable accessibilityRole="link" accessibilityLabel="Archived stories and posts" onPress={() => router.push('/archive')} style={styles.health}><Ionicons name="archive-outline" size={20} color={colors.brand}/><Text style={[styles.meta,{flex:1}]}>Archive</Text><Ionicons name="chevron-forward" size={16} color={colors.textMuted}/></Pressable>
   ...
   <SwipeSurface fill={false} enabled={!previewSection} delegateRight={tab === 'Posts'} onSwipe={swipe} onProgress={onProgress} renderPreview={direction => {
     const next = swipeDestination('/profile', tab, direction);
