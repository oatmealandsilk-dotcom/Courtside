...
export interface MediaPickerProps { compact?: boolean; value: PickedMedia | null; onChange: (next: PickedMedia | null) => void; }
export function MediaPicker({ value, onChange, compact }: MediaPickerProps) {
  ...
  return <View style={{ gap: 10 }}>
    <Pressable accessibilityRole="button" accessibilityLabel="Choose a photo or video" onPress={choose}
      ...
      <Text style={{ color: colors.text, fontSize: 16, fontWeight: '600' }}>{value ? value.label : compact ? 'Photo or video' : 'Select a photo or video'}</Text>
