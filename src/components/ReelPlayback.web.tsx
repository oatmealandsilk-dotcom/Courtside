...
export function ReelPlayback({uri,poster,active}:{uri:string;poster?:string;active:boolean}) {
 ...
 return <div style={{position:'absolute',inset:0}}>
  <video ref={video} src={uri} poster={poster} loop muted={muted} playsInline preload={active?'auto':'none'} onError={()=>setError(true)} style={{width:'100%',height:'100%',objectFit:'cover',pointerEvents:'none'}}/>
